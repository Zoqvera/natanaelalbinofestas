(() => {
  ...
  const conversionRate = (conversions, visitors) => {
    const totalVisitors = numberValue(visitors);
    if (!totalVisitors) return 0;
    return (numberValue(conversions) * 100) / totalVisitors;
  };
  ...
  const renderDashboard = (data) => {
    ...
    setText(
      "metricChatgptConversion",
      formatPercent(conversionRate(report.chatgpt_clicks, report.chatgpt_visitors)),
    );
  ...
  const showLogin = (message = "Entre para visualizar os dados de conversão.", isError = false) => {
  ...
  const loadReport = async () => {
    ...
    setStatus("Atualizando funil de conversão...");
    ...
    try {
      ...
      showDashboard();
      setStatus(`Funil de conversão dos últimos ${period} dias.`);
    } catch (error) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      console.error("Falha no login do painel de conversão:", error);
  ...
  initialize().catch((error) => {
    console.error("Falha ao iniciar painel de conversão:", error);
    showLogin("Não foi possível iniciar o painel de conversão.", true);
  });
})();
